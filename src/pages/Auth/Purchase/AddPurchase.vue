<template>
	<div>
		...
		<div class="grid-container">
			...
			<div class="grid-item">
				<b-form-checkbox size="lg" value="1" v-model="local_purchase"></b-form-checkbox>
			</div>
			<div class="grid-item"></div>
		</div>
		...
		<b-row class="ml-1">
			<b-button variant="success" @click.prevent="savePurchase" :disabled="loading">
				<i class="fa fa-floppy-o" aria-hidden="true"></i> Save
			</b-button>
			<b-button variant="danger" class="ml-2" @click.prevent="goBack" :disabled="loading">
			...
			<b-row>
				<b-col>
					<div>
						...
						<select class="custom-select" @change="onLocationChangeHandler" v-model="location_id">
							<option selected disabled value="null">Select Location</option>
							<option v-for="location in locations" :key="location.id" :value="location.id" :disabled="editMode">
								{{ location.name }}
							</option>
						</select>
					</div>
				...
				<b-col>
					<div class="form-group">
						...
						<input type="number" class="form-control" id="price" :disabled="!item_id" v-model="price" />
					</div>
				</b-col>
				<b-col>
					<div class="form-group">
						<label for="quantity">Quantity:</label>
						<input type="number" class="form-control" id="quantity" :disabled="!price" v-model="quantity" />
					</div>
				</b-col>
			</b-row>
			<b-row class="ml-1">
				<b-button type="submit" variant="success" class="admin-add-user-add-button" :disabled="!price || !quantity">
		...
		<b-container class="">
			<b-row>
				<ItemsTable :records="details" @itemToDelete="removeRecord" @itemToUpdate="editRecord" />
			</b-row>
		</b-container>
	</div>
</template>
...
export default {
	...
	computed: {
		...
		similarItemSelected() {
			const index = this.details.findIndex(
				detail => detail.location_id === this.location_id && detail.item_id === this.item_id
			);
	...
	methods: {
		...
		onLocationChangeHandler(e) {
			...
			const selectedLocation = this.locations.find(location => location.id === parseInt(locationId));
		...
		submitForm() {
			...
			const index = this.details.findIndex(i => i.location_id === this.location_id && i.item_id === this.item_id);
			...
			this.total_amount = this.details.map(d => d.total_price).reduce((prev, next) => prev + next);
			this.resetForm();
		},
		resetForm() {
			this.location = '';
		...
		removeRecord(index) {
			...
			this.$swal
				...
				.then(result => {
					if (result.value) {
						this.total_amount = this.total_amount - this.details[index].total_price;
